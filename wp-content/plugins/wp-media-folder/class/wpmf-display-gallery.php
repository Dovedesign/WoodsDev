...
class Wpmf_Display_Gallery {
    ...
    function wpmf_gallery_scripts() {
        ...
        wp_register_style('wpmf-flexslider-style', plugins_url('assets/css/display-gallery/flexslider.css', dirname(__FILE__)), array(), '2.0.0');
        ...
        wp_register_script('wpmf-gallery', plugins_url('assets/js/display-gallery/site_gallery.js', dirname(__FILE__)), array('jquery', 'wordpresscanvas-imagesloaded'), '', true);
    }
    
    function wpmf_gallery_enqueue_admin_scripts() {
        $screen = get_current_screen();
        if ('post' == $screen->base) {
            wp_register_script('wpmf-gallery-admin-js', plugins_url('assets/js/display-gallery/admin_gallery.js', dirname(__FILE__)), array('jquery'), '', true);
    ...
    function wpmf_gallery_shortcode($blank, $attr) {
        ...
        wp_enqueue_style('wpmf-gallery-style', plugins_url('/assets/css/display-gallery/style-display-gallery.css', dirname(__FILE__)));
    ...
    function wpmf_gallery_print_media_templates() {
        ...
        <script type="text/html" id="tmpl-wpmf-gallery-settings">
            ...
            <label class="setting">
                <select class="autoinsert" name="autoinsert" data-setting="autoinsert">
                    <option value="0" selected><?php echo _e('No','wpmf'); ?></option>
                    <option value="1"  ><?php echo _e('Yes','wpmf'); ?></option>
                </select>
            </label>
            
            <input type="text" class="folder_id"  data-setting="folder_id" style="display: none">
        </script>
        <?php
    }
    ...
    function delete_attachment($pid){
    ...
    function update_post($post_ID,$post_after, $post_before){
    ...
    function wpmf_auto_insert_gallery_folder_1($gallery) {
        ...
        $sql = $wpdb->prepare("SELECT * FROM $table_term_relationships WHERE term_taxonomy_id IN (%s)",array($gallery['folder_id']));
    ...
    function upload_update_post($gallery,$ids_old_array){
        $folder_ids = explode(',', $gallery['folder_id']);
        $img_auto_0 = array();
        $img_auto_1 = array();
        $img_auto = array();
        foreach ($folder_ids as $folder_id){
            if(isset($folder_id) && $folder_id !='' ){
                if($folder_id !=0){
                    $img_auto_1 = $this->wpmf_auto_insert_gallery_folder_1($gallery);
                }else{
