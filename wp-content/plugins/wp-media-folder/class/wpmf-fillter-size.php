...
class Wpmf_Fillter_Size {
    ...
    function __construct() {
        ...
        add_action('wp_ajax_wpmf_folder_order', array($this, 'folder_order') );
    ...
    public function addStyleFillterOrder() {
        wp_enqueue_style('wpmf-style_fillter_order',plugins_url( '/assets/css/style_fillter_order.css', dirname(__FILE__) ));   
    }
    
    public function wpmf_load_custom_wp_admin_script() {
            wp_register_script('wpmf-fillter-size', plugins_url( '/assets/js/fillter-size.js', dirname(__FILE__) ),array('plupload'));
    ...
    public function wpmf_fillter_size_weight($query) {
        ...
        $searchall = get_option('wpmf_searchall');
        if(isset($searchall)){
            if(($searchall == 1 && !$_REQUEST['query']['s']) || $searchall == 0){
    ...
    public function filter_size($where) {
        global $wpdb;
        $id = $_SESSION['id_post'];
    ...
    public function wpmf_get_size($sizes,$weights) {
        ...
        add_filter('posts_where', array($this, 'filter_size'));
    }
    
    public function folder_order(){
