...
spl_autoload_register('cpmb_Meta_Box::autoload_helpers');
...
$meta_boxes = array();
$meta_boxes = apply_filters( 'cpmb_meta_boxes' , $meta_boxes );
foreach ( $meta_boxes as $meta_box ) {
	$my_box = new cpmb_Meta_Box( $meta_box );
}
 ...
 */
function get_cpmb_meta_box_url() {

	if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
		// Windows
		$content_dir = str_replace( '/', DIRECTORY_SEPARATOR, WP_CONTENT_DIR );
		$content_url = str_replace( $content_dir, WP_CONTENT_URL, dirname(__FILE__) );
		$cpmb_url = str_replace( DIRECTORY_SEPARATOR, '/', $content_url );

	} else {
	  $cpmb_url = str_replace(
	...
	return trailingslashit( apply_filters('cpmb_meta_box_url', $cpmb_url ) );
}

define( 'CPMB_META_BOX_URL', get_cpmb_meta_box_url() );
...
class cpmb_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		$allow_frontend = apply_filters( 'cpmb_allow_frontend', true, $meta_box );
		...
		$show_filters = 'cpmb_Meta_Box_Show_Filters';
		foreach ( get_class_methods( $show_filters ) as $filter ) {
			add_filter( 'cpmb_show_on', array( $show_filters, $filter ), 10, 2 );
		}

		// register our scripts and styles for cpmb
	...
	function register_scripts() {
		...
		$scripts = array( 'jquery', 'jquery-ui-core', 'jquery-ui-datepicker', /*'media-upload', */'cpmb-timepicker' );
		...
		wp_localize_script( 'cpmb-scripts', 'cpmb_l10', array(
	...
	/**
	 * Enqueues scripts and styles for CPMB
	 * @since  1.0.0
	 */
	function do_scripts( $hook ) {
		// only enqueue our scripts/styles on the proper pages
		if ( $hook == 'post.php' || $hook == 'post-new.php' || $hook == 'page-new.php' || $hook == 'page.php' ) {
			wp_enqueue_script( 'cpmb-scripts' );

			// default is to show cpmb styles on post pages
			if ( ! isset( $this->_meta_box['cpmb_styles'] ) || $this->_meta_box['cpmb_styles'] != false )
				wp_enqueue_style( 'cpmb-styles' );
		}
	}
	...
	function add_metaboxes() {
		...
		foreach ( $this->_meta_box['pages'] as $page ) {
			if ( apply_filters( 'cpmb_show_on', true, $this->_meta_box ) )
	...
	function user_metabox() {
		...
		// default is to NOT show cpmb styles on user profile page
		if ( isset( $this->_meta_box['cpmb_styles'] ) && $this->_meta_box['cpmb_styles'] == true  )
			wp_enqueue_style( 'cpmb-styles' );
	...
	public static function save_fields( $meta_box, $object_id, $object_type = '' ) {
		...
		if ( ! apply_filters( 'cpmb_show_on', true, $meta_box ) )
			return;
		...
		foreach ( $meta_box['fields'] as $field ) {
			...
			$new = apply_filters( 'cpmb_validate_'. $field['type'], $new, $object_id, $field, $object_type );

			// Allow validation via metabox flag
			if ( isset( $field['validate_func'] ) ) {
				$ok = call_user_func( array( 'cpmb_Meta_Box_Validate', $field['validate_func'] ), $new );
		...
		do_action( "cpmb_save_{$object_type}_fields", $object_id, $meta_box['id'], $updated, $meta_box );
...
// Handle oembed Ajax
add_action( 'wp_ajax_cpmb_oembed_handler', array( 'cpmb_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_cpmb_oembed_handler', array( 'cpmb_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @param int   $object_id  Object ID
 */
function cpmb_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
		cpmb_print_metabox( $meta_box, $object_id );
	}
}
 ...
 * @param int   $object_id Object ID
 */
function cpmb_print_metabox( $meta_box, $object_id ) {
	$cpmb = new cpmb_Meta_Box( $meta_box );
	if ( $cpmb ) {

		cpmb_Meta_Box::set_object_id( $object_id );

		if ( ! wp_script_is( 'cpmb-scripts', 'registered' ) )
			$cpmb->register_scripts();

		wp_enqueue_script( 'cpmb-scripts' );

		// default is to show cpmb styles
		if ( ! isset( $meta_box['cpmb_styles'] ) || $meta_box['cpmb_styles'] != false )
			wp_enqueue_style( 'cpmb-styles' );

		cpmb_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function cpmb_save_metabox_fields( $meta_box, $object_id ) {
	cpmb_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 * @return string             CPMB html form markup
 */
function cpmb_metabox_form( $meta_box, $object_id, $echo = true ) {

	// Make sure that our object type is explicitly set by the metabox config
	cpmb_Meta_Box::set_object_type( cpmb_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		// check nonce
		isset( $_POST['submit-cpmb'], $_POST['object_id'], $_POST['wp_meta_box_nonce'] )
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], cpmb_Meta_Box::nonce() )
		&& $_POST['object_id'] == $object_id
	)
		cpmb_save_metabox_fields( $meta_box, $object_id );
	...
	$form_format = apply_filters( 'cpmb_frontend_form_format', '<form class="cpmb-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-cpmb" value="%s"></form>', $object_id, $meta_box, $form );

	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save', 'cpmb' ) );
